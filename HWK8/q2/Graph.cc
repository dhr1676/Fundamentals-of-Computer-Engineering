...
void Graph::Print_2() {
    std::cout << "Printing graph:\n";
    for (int i = 0; i < size; i++) {
        std::cout << "Vertex " << v_name[i] << ":";
        if (V[i].parent >= 0)
            std::cout << " parent=" << v_name[V[i].parent];
        if (V[i].distance < INT_MAX)
            std::cout << " distance=" << V[i].distance;
        if (V[i].time_in >= 0)
            std::cout << " time_in=" << V[i].time_in;
        ...
        std::cout << ", edges={";
        for (Edge *edge = V[i].edges; edge; edge = edge->next)
            std::cout << ' ' << v_name[i] << "->" << v_name[edge->v];
        std::cout << " }\n";
    }
}
...
void Graph::PrintShortestPath_2(int u, int v) {
    if (u == v) {
        std::cout << v_name[u];
    } else if (V[v].parent == -1) {
        std::cout << "unreachable";
    } else {
        PrintShortestPath_2(u, V[v].parent);
        std::cout << " -> " << v_name[v];
    }
}
...
//            Print();
        ...
        std::cout << "iteration time  " << i + 1 << '\n';
        for (int k = 0; k < size; k++) {
            std::cout << "Shorted path from s to " << v_name[k] << ": ";
            PrintShortestPath_2(0, k);
            std::cout << '\n';
        }
        std::cout << '\n';
    }
