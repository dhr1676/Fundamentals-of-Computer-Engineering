...
//    改当前node的parent和node->left的关系
    if (node->parent == NULL) {
        this->root = node->left;
        node->left->parent = NULL;                  //一定记得改node->left的parent的关系
    } else {
//        如果当前node是他parent的左子
...
//        把当前node的left变成当前node的parent,这步很重要,先别动node,用temp来做
//        不做操作了,等全部换完,再把这步补上,此时node还是node,只不过x现在是root
//        connection to node's parent is broken
//        或者如果不是root,那就是node的parent到x的关系转接完毕
